 ...
 * Пример 1.
 * класс ExampleViolationOne является подклассом Example и переопределяет его метод,
 * усиливая одно из его предусловий
 * в итоге мы получим аварийную ситуацию, когда при подаче воды будем использовать
 * класс ExampleViolationOne,
 ...
 * Это вызывает ситуацию, когда используя объект класса ExampleViolationTwo для подачи воды,
 * мы можем дать давление недостаточное для того, чтобы эта воды пошла,
 * но метод нас об этом не будут оповещать.
 *
 * Пример 3.
 ...
 * Это может вызвать аварийную ситуацию, т.к. теперь мы вообще не проверяем давление,
 * под которым подаем водуБ
...
public class Example {
    ...
    static class ExampleViolationOne extends Example {
        ...
        public boolean getWater(int waterPressure) {
            boolean result = true;
            if (getDiameter() < 10 && waterPressure < 5 || waterPressure > 13) {
                result = false;
            }
            return result;
        }
    ...
    static class ExampleViolationTwo extends Example {
        ...
        public boolean getWater(int waterPressure) {
            boolean result = true;
            if (getDiameter() < 10 && waterPressure < 4 || waterPressure > 10) {
                result = false;
            }
            return result;
        }
